        ...
        runScenario(
            TestScenario(
                ...
                PropagateAndRollback(
                    "no restrictions, so no propagation",
                    Nil,
                    cs.map(c => DomainReduction(c, c.domain))
                        .concat(xs.map(x => DomainReduction(x, x.domain)))
                        .appended(DomainReduction(y, y.domain))),
                PropagateAndRollback(
                    ...
                    List((c1, CompleteBooleanDecisionDomain), (c2, CompleteBooleanDecisionDomain), (c3, CompleteBooleanDecisionDomain),
