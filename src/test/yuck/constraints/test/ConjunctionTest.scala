...
final class ConjunctionTest extends UnitTest with ConstraintTestTooling {
    ...
    private val constraint = new Conjunction(space.nextConstraintId(), xs, y)
    ...
    def testCopyingWithoutReplacement(): Unit = {
        val copy = constraint.copy(Map.empty).asInstanceOf[Conjunction]
    ...
    def testCopyingWithReplacement(): Unit = {
        ...
        val copy = constraint.copy(Map((y, y1))).asInstanceOf[Conjunction]
        assertEq(copy.id, constraint.id)
        assertEq(copy.xs, xs)
        assertEq(copy.y, y1)
    }
