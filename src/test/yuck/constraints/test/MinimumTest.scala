...
final class MinimumTest extends UnitTest with ConstraintTestTooling {
    ...
    @Test
    def testBasics(): Unit = {
        val constraint = new Minimum(space.nextConstraintId(), xs, y)
    ...
    def testCopyingWithoutReplacement(): Unit = {
        val constraint = new Minimum(space.nextConstraintId(), xs, y)
        val copy = constraint.copy(Map.empty).asInstanceOf[Minimum[?, ?, ?]]
    ...
    def testCopyingWithReplacement(): Unit = {
        ...
        val copy = constraint.copy(Map((y, y1))).asInstanceOf[Minimum[?, ?, ?]]
        assertEq(copy.id, constraint.id)
        assertEq(copy.xs, xs)
    ...
    private def testPropagation(xs: Seq[IntegerVariable]): Unit = {
        space.post(new Minimum(space.nextConstraintId(), xs, y))
        runScenario(
            TestScenario(
                space,
