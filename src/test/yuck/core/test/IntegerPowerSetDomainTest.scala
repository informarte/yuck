       ...
       with IntegerSetDomainTestDataFactory
{
    ...
    def testBasics(): Unit = {
        ...
        assertThrows(esd.nextRandomValue(randomGenerator, es))
        ...
        assert(! usd.isEmpty)
        assertThrows(usd.size)
        assert(usd.isComplete)
        assert(! usd.isFinite)
        assert(! usd.isSingleton)
        List(es, us, s0, s1, s2, s01, s02).foreach(s => assert(usd.contains(s)))
        assertThrows(usd.singleValue)
        assertThrows(usd.values)
        assertThrows(usd.valuesIterator)
        assertThrows(usd.randomValue(randomGenerator))
        assertThrows(usd.nextRandomValue(randomGenerator, es))
