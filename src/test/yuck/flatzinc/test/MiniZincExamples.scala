...
final class MiniZincExamples extends MiniZincBasedTest {
    ...
    def testAlpha: Unit = solve("alpha")

    @Test
    @Category(Array(classOf[HardInstance], classOf[SatisfiabilityProblem]))
    def testBattleships: Unit = solve("battleships_1")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testBlocksworld1: Unit = solve(task.copy(problemName = "blocksworld_instance_1"))

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testBlocksworld2: Unit = solve(task.copy(problemName = "blocksworld_instance_2"))

    // Search variables item[] have infinite domains but pruning saves the day.
    @Test
    @Category(Array(classOf[EasyInstance], classOf[MinimizationProblem]))
    def testCutstock: Unit = solve(task.copy(problemName = "cutstock", maybeOptimum = Some(4)))

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testEq20: Unit = solve("eq20")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testFactoryPlanning: Unit = solve("factory_planning_instance")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[MinimizationProblem], classOf[HasAlldifferentConstraint]))
    def testGolomb: Unit = solve(task.copy(problemName = "golomb", maybeOptimum = Some(6)))

    @Test
    @Category(Array(classOf[EasyInstance], classOf[MinimizationProblem]))
    def testJobshop: Unit = solve(task.copy(problemName = "jobshop2x2", maybeOptimum = Some(11)))

    @Test
    @Category(Array(classOf[HardInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testKnights: Unit = solve(task.copy(problemName = "knights"))
    ...
    def testLangford: Unit = solve("langford")
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testLangford2: Unit = solve("langford2")

    // Runtime depends strongly on algorithmic details like choice of seed and random generator.
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testLatinSquares: Unit = solve("latin_squares_fd")

    // Magic sequence instances have tight all_different constraints.
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    @Test
    def testMagicSequence3: Unit = solve("magicsq_3")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testMagicSequence4: Unit = solve("magicsq_4")

    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    @Test
    def testMagicSequence5: Unit = solve("magicsq_5")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[MaximizationProblem]))
    def testMultiDimKnapsack: Unit = solve(task.copy(problemName = "multidimknapsack_simple", maybeOptimum = Some(17)))
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testPartition: Unit = solve("partition")
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testQuasigroup: Unit = solve("quasigroup_qg5")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testQueensCp: Unit = solve("queen_cp2")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testQueensIp: Unit = solve("queen_ip")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testSimpleSat: Unit = solve("simple_sat")
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testSteinerTriples: Unit = solve(task.copy(problemName = "steiner-triples"))

    // Runtime depends strongly on algorithmic details like choice of seed and random generator.
    // Has a lot of constants in all_different constraints that are used to prune domains up-front!
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testSudoku: Unit = solve("sudoku")
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki1: Unit = solve("tenpenki_1")

    @Test
    @Category(Array(classOf[HardInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki2: Unit = solve("tenpenki_2")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki3: Unit = solve("tenpenki_3")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki4: Unit = solve("tenpenki_4")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki5: Unit = solve("tenpenki_5")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testTenpenki6: Unit = solve("tenpenki_6")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testTimetabling: Unit = solve("timetabling")
    ...
    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem]))
    def testWolfGoatCabbage: Unit = solve("wolf_goat_cabbage")

    @Test
    @Category(Array(classOf[EasyInstance], classOf[SatisfiabilityProblem], classOf[HasAlldifferentConstraint]))
    def testZebra: Unit = solve("zebra")
