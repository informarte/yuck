 ...
 * |[a[1] - b[1]| + ... + |[a[n] - b[n]|.
 *
 * The matrix is given in terms of columns to improve data locality.
 *
    ...
    extends Constraint(id, goal)
{
    ...
    private val cols = {
        ...
        val feasibleRows =
            ...
            .toIndexedSeq
        for (i <- 0 until n) yield for (j <- 0 until feasibleRows.size) yield rows(j)(i)
    }
    private val m = cols(0).size // number of rows
    private val x2i = new immutable.HashMap[AnyVariable, Int] ++ xs.zip(0 until n)
    private var currentDistances = new Array[Int](m) // for each row
    private var futureDistances = new Array[Int](m) // for each row
    private val effects = List(new ReusableEffectWithFixedVariable[IntegerValue](costs))
    private val effect = effects.head

    override def initialize(now: SearchState) = {
        for (j <- 0 until m) {
            currentDistances(j) = 0
            for (i <- 0 until n) {
