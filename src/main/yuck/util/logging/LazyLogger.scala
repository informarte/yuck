...
final class LazyLogger(logger: Logger) {
    ...
    def log(msg: => String): Unit = write(InfoLogLevel, msg)

    /** Logs on FineLogLevel - currentLogLevelReduction. */
    def logg(msg: => String): Unit = write(FineLogLevel, msg)

    /** Logs on FinerLogLevel - currentLogLevelReduction. */
    def loggg(msg: => String): Unit = write(FinerLogLevel, msg)
    
    /** Logs on FinestLogLevel - currentLogLevelReduction. */
    def logggg(msg: => String): Unit = write(FinestLogLevel, msg)
