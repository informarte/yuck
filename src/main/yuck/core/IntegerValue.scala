...
final class IntegerValue(val value: Long) extends IntegralValue[IntegerValue] {
    ...
    override def -(that: IntegerValue) = if that.value == 0 then this else IntegerValue(safeSub(this.value, that.value))
    ...
    override def ^(that: IntegerValue) = {
        ...
        if result == java.lang.Double.NEGATIVE_INFINITY || result == java.lang.Double.POSITIVE_INFINITY ||
            ...
            ! (if this.value >= 0 then result >= 0 else if that.value % 2 == 0 then result > 0 else result < 0) then
        {
    ...
    override def addAndSub(a: IntegerValue, b: IntegerValue) = {
        ...
        if delta == 0 then this else IntegerValue(safeAdd(this.value, delta))
    }
    override def addAndSub(s: IntegerValue, a: IntegerValue, b: IntegerValue) = {
        val delta = safeSub(a.value, b.value)
        if delta == 0 then this else IntegerValue(safeAdd(this.value, safeMul(s.value, delta)))
    }
    inline override def abs = if value < 0 then negated else this
...
object IntegerValue {
    ...
    inline def max(a: IntegerValue, b: IntegerValue): IntegerValue = if a > b then a else b
    ...
    def apply(a: Int): IntegerValue =
       if Lb <= a && a < Ub then valueCache(a - Lb) else new IntegerValue(a.toLong)
    ...
    def apply(a: Long): IntegerValue =
       if Lb <= a && a < Ub then valueCache(a.toInt - Lb) else new IntegerValue(a)
