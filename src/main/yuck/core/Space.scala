    ...
    checkAssignmentsToNonChannelVariables: Boolean = false)
{
    ...
    def maybeDefiningConstraint(x: AnyVariable): Option[Constraint] =
        outflowModel.get(x)

    /** Returns the constraint that computes the value of the given variable. */
    def definingConstraint(x: AnyVariable): Constraint =
        outflowModel(x)
    ...
    def isObjectiveVariable(x: AnyVariable): Boolean =
    ...
    def isOutputVariable(x: AnyVariable): Boolean =
    ...
    def isImplicitConstraint(constraint: Constraint): Boolean =
