...
import yuck.util.arm.{ManagedExecutorService, Sigint, scoped}
import yuck.util.logging.LazyLogger

/**
 * Provides FlatZinc parsing with interrupt handling.
 */
final class FlatZincParser
    (fznFilePath: String, logger: LazyLogger, sigint: Sigint)
    extends Callable[FlatZincAst]
{
    ...
    override def call() = {
        ...
        class FlatZincParserRunner extends Callable[FlatZincAst] {
            override def call() = {
                ...
                yuck.flatzinc.parser.FlatZincParser.parse(input)
            }
        }
        val threadPool = Executors.newFixedThreadPool(1)
        val futureAst = threadPool.submit(new FlatZincParserRunner)
        val ast = scoped(new ManagedExecutorService(threadPool, logger)) {
            futureAst.get
        }
