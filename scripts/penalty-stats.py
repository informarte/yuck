...
def compareRuns(cursor, args):
    ...
    jobQuery = 'SELECT DISTINCT problem, instance, problem_type FROM result WHERE run IN (%s) ORDER BY problem, instance' % ','.join('?' for run in runsInScope)
    ...
    jobs = [(problem, instance, problemType)
            for (problem, instance, problemType) in cursor.execute(jobQuery, runsInScope)
