...
def computeGaps(cursor, args):
    jobQuery = 'SELECT DISTINCT problem, instance, problem_type FROM result WHERE run IN (%s) ORDER BY problem, instance' % ','.join('?' for run in args.runs)
    problemPattern = re.compile(args.problemFilter)
    instancePattern = re.compile(args.instanceFilter)
    jobs = [(problem, instance, problemType)
            for (problem, instance, problemType) in cursor.execute(jobQuery, args.runs)
    ...
    for run in args.runs:
        resultQuery = 'SELECT problem, instance, solved, quality FROM result WHERE run = ?';
        for result in cursor.execute(resultQuery, (run,)):
            (problem, instance, solved, quality) = result
            if problemPattern.match(problem) and instancePattern.match(instance):
    ...
    for (problem, instance, problemType) in jobs:
        if problemType != 'MIN':
        ...
        optima = [int(result[0]) for result in cursor.execute('SELECT optimum FROM result WHERE problem = ? AND instance = ? AND optimum IS NOT NULL', task)]
        qualities += optima
        highScores = [int(result[0]) for result in cursor.execute('SELECT high_score FROM result WHERE problem = ? AND instance = ? AND high_score IS NOT NULL', task)]
        qualities += highScores
        bks = None if not qualities else min(qualities)
        if not bks:
            raise ValueError('{}/{}: No best known solution'.format(problem, instance))
        if bks < 0:
            raise ValueError('{}/{}: Best known solution has negative objective value'.format(problem, instance))
        if args.verbose:
            print('-' * 80)
            print(problem, instance, problemType, bks)
        for run in results:
            if task in results[run]:
                ...
                if solved:
                    if not quality:
                        raise ValueError('{}/{}/{}: No objective value'.format(run, problem, instance))
                    if quality < 0:
                        raise ValueError('{}/{}/{}: Negative objective value'.format(run, problem, instance))
                    gap = quality / bks
                    if args.verbose:
