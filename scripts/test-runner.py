...
def main():
    ...
    if args.mode == 'SINGLE_JVM':
        result = subprocess.run(['./mill', '--disable-ticker', 'yuck.test.run', testClass])
        exitCode = result.returncode
        os.sync()
    else:
        result = subprocess.run(
            ['./mill', '--disable-ticker', 'yuck.test.run', '--list-test-methods', 'true', testClass],
        ...
        if exitCode == 0:
            ...
            for testMethod in testMethods:
                print(testMethod)
                result = subprocess.run(['./mill', '--disable-ticker', 'yuck.test.run', testMethod])
